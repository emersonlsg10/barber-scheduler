...
import ProfileType from 'components/timelineComponents/ProfileType';
import CardTimeLine from 'components/timelineComponents/CardTimeLine';
import IsFriendButton from 'components/timelineComponents/IsFriendButton';
          ...
          <Grid
            ...
            <Grid item lg={1}>
              <div className={classes.avatar}>
                <Avatar
                  alt="Remy Sharp"
                  src={data.user_profile?.user.images?.original}
              ...
              <div className={classes.nameUser}>
                <div style={{ fontSize: 25, marginBottom: 5 }}>
                  ...
                  <ProfileType
                    ...
                    isPublic={data.user_profile?.user.is_public_profile}
                  />
                  <div className={classes.infoUser}>
                    <span>
                      Código:{' ' + data.user_profile?.user.code_promo}
                    </span>
                    {/* <AddFriendButton
                      currentFriend={currentFriend}
                      setCurrentFriend={setCurrentFriend}
                      currentSolicited={currentSolicited}
                      setCurrentSolicited={setCurrentSolicited}
                      friendID={data.user_profile?.user?.id}
                    /> */}
                    {user?.id !== data.user_profile?.user?.id &&
                    data.user_profile?.user?.is_friend !== undefined ? (
                      <>
                        <IsFriendButton
                          codePromo={data.user_profile?.user?.code_promo}
                          isFriend={data.user_profile?.user?.is_friend}
                          wasSolicited={
                            data.user_profile?.user
                              ?.is_friendship_requested_status
                          }
                          friendID={data.user_profile?.user?.id}
                        />
                      </>
          ...
          {(!otherProfile ||
            ...
            <Grid container spacing={1} style={{ maxWidth: '100%' }}>
              <Grid item lg={12} style={{ maxWidth: '100%' }}>
                <TimeLineTitleSearch otherProfile={otherProfile} />
                <TimeLine
                  data={data?.user_profile?.time_line}
                  otherProfile={otherProfile}
